...
according to the hierarchical clustering. It is also possible to set row and column order by setting `heatmapRow` and `heatmapCol` properties.
...
lc_heatmap(dat(
  ...
  on_labelClickRow = function(i) {
    ord <- order(cor(iris[i, 1:4], iris[, 1:4]), decreasing = T)
    updateChart()
  }
))
```
