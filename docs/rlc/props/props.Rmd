...
first column is always where to put ticks. Next columns are optional and they allow to specify with what to replace tick values.
...
`width` and `height` specify size of the chart in pixels (default value for both is 500). `paddings` allows to set 
