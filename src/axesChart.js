...
export function axesChart() {
   ...
   chart.add_property("singleScaleX", true)//not active now
      ...
      .add_property("axisTitlePos", {x: {across: "up", along: "end"},
                                     y: {across: "up", along: "end"} }, 
         function(value) {
            var newValue = {x: {across: "up", along: "end"},
               y: {across: "up", along: "end"} };
            if(value.x) {
               if(!value.x.split)
                  throw "Error in 'typeCheck' for property 'axisTitlePos': " +
                           "'x' must be a string.";
               var x = value.x.split(" ").map(el => el.toLowerCase());
               if(x.indexOf("down") > -1)
                  newValue.x.across = "down";
               if(x.indexOf("middle") > -1)
                  newValue.x.along = "middle";
               if(x.indexOf("start") > -1)
                  newValue.x.along = "start";
            }
            if(value.y) {
               if(!value.y.split)
                  throw "Error in 'typeCheck' for property 'axisTitlePos': " +
   ...
   chart.updateAxesTitles = function() {

      var labels = {x: {x: 5, y: -5, anchor: "start", dy: 0},
                     y: {x: 0, y: 6, anchor: "end", dy: "1.8ex"}};
      if(chart.axisTitlePos().x.along == "middle") {
         labels.x.x = chart.plotWidth()/2;
         labels.x.anchor = "middle";
      }
      if(chart.axisTitlePos().x.along == "end") {
         labels.x.x = chart.plotWidth();
         labels.x.anchor = "end";
      }
      if(chart.axisTitlePos().x.across == "down") {
         chart.axes.x_label.text("");
         labels.x.y = chart.axes.x_g.node().getBBox().height;
         labels.x.dy = "1.8ex";
      }

      if(chart.axisTitlePos().y.along == "middle") {
         labels.y.x = -chart.plotHeight()/2;
         labels.y.anchor = "middle";
      }
      if(chart.axisTitlePos().y.along == "start") {
         labels.y.x = -chart.plotHeight();
         labels.y.anchor = "start";
      }
      if(chart.axisTitlePos().y.across == "down") {
         chart.axes.y_label.text("");
         labels.y.y = -chart.axes.y_g.node().getBBox().width;
         labels.y.dy = -6;
      }

      chart.axes.x_label
         .html(chart.axisTitleX());
      chart.axes.y_label
         .html(chart.axisTitleY());
      ...
      if(chart.transitionDuration() > 0 && !chart.transitionOff){
         ...
         chart.axes.x_label.transition(t)
            ...
            .attr("y", labels.x.y)
            .attr("dy", labels.x.dy)
            .style("text-anchor", labels.x.anchor);

         chart.axes.y_label.transition(t)
            .attr("x", labels.y.x)
            .attr("y", labels.y.y)
            .attr("dy", labels.y.dy)
            .style("text-anchor", labels.y.anchor);
      }  else {
         chart.axes.x_label
            .attr("x", labels.x.x)
            .attr("y", labels.x.y)
